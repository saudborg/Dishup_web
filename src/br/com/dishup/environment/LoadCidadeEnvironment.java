...
public class LoadCidadeEnvironment {
	 ...
	 ***********************************/
	public void carregaCidade(String filePath){
		...
		statistic.start();
		ArrayList<Cidade> listFile = new ArrayList<Cidade>();
		...
		try{
			...
			ArrayList<Cidade> listBase = new ArrayList<Cidade>();;
			try {
				listBase = cidadeDAO.selectAllOrderById();
				...
				while((countListBase < numberOfRegisterBase) || (countListFile < numberOfRegisterFile)){
					if((countListBase < numberOfRegisterBase) && (countListFile < numberOfRegisterFile)){
						if(listFile.get(countListFile).getId() == listBase.get(countListBase).getId()){
							...
							cidadeDAO.deleteById(listBase.get(countListBase).getId());
							statistic.incrementRegisterDeleted();
							countListBase++;
						}else{
							cidadeDAO.insert(listFile.get(countListFile));
				...
				for(int i = 0; i < listFile.size(); i++){
					try{
						...
						cidadeDAO.insert(listFile.get(i));
						statistic.incrementRegisterWrite();
						...
						System.out.println(e1.getMessage());
					}catch (CidadeAlreadyExistException e1) {
						System.out.println(e1.getMessage());
					}
				}
				statistic.end();
				...
				System.out.println(e.getMessage());
			} catch (CidadeAlreadyExistException e) {
				System.out.println(e.getMessage());
			}
	 ...
	 ********************************/
	private ArrayList<Cidade> loadFileIntoArray(File file) throws FileNotFoundException, FileEmptyException{
		...
		while(scanner.hasNext()){
			try{
				...
				parms = var.split(";");
				listaCidade.add(new Cidade(Integer.valueOf(parms[0]), parms[1], estadoDAO.selectBySigla(parms[3]), paisDAO.selectBySigla(parms[2])));
